...
export const BASE_RADII = [0.44, 0.37, 0.47, 0.34, 0.42, 0.31, 0.46, 0.36];
 ...
 */
export function buildBlobPath(radii) {
  const n = BASE_ANGLES.length;
  const pts = BASE_ANGLES.map((deg, i) => {
    const rad = (deg * Math.PI) / 180;
    return { x: 0.5 + radii[i] * Math.cos(rad), y: 0.5 + radii[i] * Math.sin(rad) };
  });
